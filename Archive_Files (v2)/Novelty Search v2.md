...
| 3. M2+M3 Gated Feature-Fusion Ensemble | ❌ **DROPPED — failed the §4.0 admission test** | Re-run as `Asif's/M30_v2/` on the same split and metric as its backbones, with M2-alone and M3-alone evaluated on the *identical* test cycles. Result: fusion **0.5975** vs **M2-alone 0.6098** — the fusion *loses* to a single backbone by −0.0123. §4.0 admits this item **only if it measurably beats both backbones alone; drop it silently if it doesn't.** It doesn't. The original's "+9.86% over M2" was an artifact of comparing across a metric boundary *and* a split boundary. Per §4.0, **swap in a deferred item — do not accumulate.** The selected set is now **2 items, not 3**. Sanity anchors were clean (M2 0.6098 vs published 0.6138; M3 0.5853 vs 0.5895), so this is a trustworthy negative, not a broken run. |
