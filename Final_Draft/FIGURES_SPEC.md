...
|---|---|---|---|---|
| F1 | `fig:flowchart` | 🔴 **to draw** | `figures/fig_flowchart.pdf` | by hand (draw.io / TikZ) |
...
F1 is the only figure the paper currently renders as a placeholder box. F6 and F7 are copied into
`figures/` and are ready to insert if page budget allows; the paper compiles without them.

---

## F1 — System flowchart *(the one that must be drawn)*

**Placement:** Section II, immediately after the opening paragraph. Replace the `\framebox` line
with `\includegraphics[width=0.92\linewidth]{figures/fig_flowchart.pdf}`.

**Caption already written in the .tex** — do not restate it inside the figure.

**Required content, top to bottom.** Two blocks must be visually heavier (bold border or shaded)
...
**Style notes.** Single column, so aim for ~85 mm tall at 0.92\linewidth. Greyscale-safe (the
report may be printed). Label the two heavy boxes with the same words the caption uses
("audited split loader", "official scoring module") so the reader can match them.
...
## Reproduction commands, all four existing generators
