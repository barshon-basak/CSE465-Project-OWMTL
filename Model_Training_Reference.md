...
- **Chunk:** E (Novelty Layer). **Requires:** M2 and M3 (real checkpoints). **Status:** 🔴 **result withdrawn — needs re-export.** Fuses 768-dim M2 (CNN) and 1280-dim M3 (MobileNetV2) frozen features via a Gated Adaptive Fusion (GAF) head. **Selected Novelty Item #3** — see `Novelty Search.md` §4.4.
  ...
  - **Re-run is built and tested, not yet executed:** `Asif's/M30_v2/` (generated by `gen_M30_v2.py`, 53/53 tests including three failure injections). Differences from the original are confined to the split, the metric reporting, and checkpoint-loading safety — architecture and hyperparameters are unchanged. It refuses to run on randomly-initialised backbones (`load_backbone_or_die()`), and evaluates M2-alone and M3-alone on the *same* test cycles so `Novelty Search.md` §4.0's "only if it beats both backbones alone" admission test is actually run rather than assumed. Verdict lands in `best_metrics.admission_test`; a **FAILS** verdict is a legitimate result and means the item gets dropped, not retried. Needs: M2 + M3 `best_model.pth` uploaded to `/content/`, a Kaggle key, ~20–30 min on a T4.
