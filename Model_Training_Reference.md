...
- **Chunk:** E (Novelty Layer). **Requires:** M2 and M3 (real checkpoints). **Status:** ❌ **DROPPED — failed the §4.0 admission test.** Gated Adaptive Fusion over frozen M2 (768-d) + M3 (1280-d) embeddings.
  - **Verdict (2026-08-14, `Asif's/M30_v2/`):** on identical test cycles, fusion **0.5975** vs **M2-alone 0.6098** and **M3-alone 0.5853**. The fusion *loses to a single backbone* by −0.0123. `Novelty Search.md` §4.0 admits this item only if it beats both backbones alone, so it is dropped and a deferred item should be swapped in.
  - **Why the original said otherwise:** `Barshon's/M30/results_M30.json` reported ICBHI 0.8213 and "+9.86% over M2". That number was the **legacy macro metric**, on a **random 70/30 split** (its `build_icbhi_splits()` docstring claimed the official split while the body shuffled), with **no committed `confusion_matrix_raw`**. The claimed boost compared across a metric boundary *and* a split boundary.
  - **The negative is trustworthy.** M30 v2's sanity anchors reproduced the published backbone scores closely (M2 0.6098 vs 0.6138; M3 0.5853 vs 0.5895), and its checkpoint loading is asserted rather than best-effort, so the result is not an artifact of a mis-loaded backbone.
  - **Reportable finding:** heterogeneous feature fusion of a scratch CNN and an ImageNet-pretrained MobileNetV2 does **not** help on this task — a clean negative that the original's cross-split comparison concealed.
=======
