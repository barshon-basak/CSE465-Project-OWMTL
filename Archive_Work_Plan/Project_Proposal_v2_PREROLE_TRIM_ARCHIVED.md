...
**Revision note (v2):** This version adds a mandatory, project-wide **Standardized Model Training & Reporting Protocol** (§8.5) — full metric suites, efficiency reporting, learning curves, and a data-augmentation ablation applied identically by all four members — per the supervisor's explicit instruction. The core narrative, novelty claim, dataset plan, and open-world mechanism (§§1–7) are unchanged from v1; this is a rigor/reporting upgrade, not a scope change, and it strengthens the paper's Q1 competitiveness by closing a gap reviewers routinely flag: incomplete efficiency reporting and missing augmentation ablations in ICBHI-based papers.
...
| **With vs. without data augmentation, per architecture, per member** | All tiers | Full — mandatory, see §8.5.3 |

---

## 8.5 Standardized Model Training & Reporting Protocol *(supervisor mandate — applies to every model, every member)*
...
- **Confusion matrix** — raw counts and row-normalized, for every model and every relevant train/test split (including per-OWL-stage matrices for Member B, and pre-/post-compression matrices for Member C).
...
- **Inference latency** (ms/sample, batch size 1) — required in addition to the above for Member C's compression work, and recommended for all members since it strengthens the paper's "real-world deployability" narrative.
...
### 8.5.4 Required data-augmentation ablation (every member, own architectures, own data)
...
- **Member A** (backbone/sound-event, cycle-level spectrograms): SpecAugment (time/frequency masking) as the primary technique, following standard AST-based ICBHI practice; report as an additional axis in the existing architecture × input-representation ablation grid.
- **Member B** (disease-diagnosis head, patient-level, small known-class counts): class-balanced augmentation targeting the smaller known classes (Healthy n=26, URTI n=14) — noise injection, pitch-shift, time-stretch on cycle-level audio before patient-level aggregation — evaluated on whether it improves known-class balance *without* distorting the cross-task disagreement signal (i.e., re-check the unknown-detection numbers post-augmentation, not just known-class accuracy).
- **Member C** (cross-dataset OOD + compression): augmentation used as a **domain-robustness lever**, not just a class-balance lever — mild simulated channel/device noise on ICBHI training data to test whether it narrows the accuracy gap on Coswara/SPRSound, reported alongside the existing compression ratio vs. accuracy-retention curve.
- **Member D** (calibration/trust): augmentation is evaluated for its effect on the *trust* metrics, not raw accuracy — does augmenting the training set change calibration error and conformal coverage, in-distribution and under Coswara/SPRSound shift? This is a natural, non-redundant extension of Member D's existing calibration-under-shift experiment.
- All four members use a **held-out, patient-independent validation/calibration split untouched by augmentation** (augmentation is applied to training folds only), consistent with the existing patient-independent evaluation protocol (§7).
...
15. Park, D.S., Chan, W., Zhang, Y., et al. (2019). SpecAugment: A Simple Data Augmentation Method for Automatic Speech Recognition. *Interspeech 2019*. [Standard masking-based augmentation adopted by Member A; widely used in AST-based ICBHI pipelines.]
16. Kim, J.-W., Toikkanen, M., Bae, S., Kim, M., Jung, H.-Y. (2024). RepAugment: Input-Agnostic Representation-Level Augmentation for Respiratory Sound Classification. arXiv:2405.02996. [Reports up to 7.14% accuracy improvement on minority ICBHI disease classes; candidate augmentation technique for Member B's small known-class balancing.]
17. Bae, S., Kim, J.-W., et al. — noise injection / time-shift / stretch / pitch-shift as standard ICBHI augmentation baselines, as surveyed in multiple 2024–2026 ICBHI robustness studies. [Candidate for Member C's domain-robustness augmentation lever; verify primary source before submission.]
...
- [ ] Verify novelty claim is still current at time of submission — a July 2026 literature check found no published open-world MTL + cross-task-disagreement + staged-OWL work on respiratory audio, and no conformal/distribution-free open-set guarantee applied to respiratory sound (§4, §4 of Member D's whitespace argument); re-check shortly before submission since this is a fast-moving area
- [ ] Confirm reference #17 (augmentation baseline survey) with a verified primary citation before submission
- [ ] All four members' Model Zoo & Reporting Appendix tables (§8.5) reconciled into one consistent schema before manuscript drafting begins (Week 16–17, not left until Week 18–21)
