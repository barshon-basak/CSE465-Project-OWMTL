...
| **E** — Novelty layer | 2–3 selected items, fixed (`Novelty Search.md` §4.0) | ⚠️ **Partly blocked.** M13 prototypical head real. M14 conformal real but AUROC 0.4809 (below chance) and 0% unknown detection. 🔴 M30 fusion **withdrawn** — no confusion matrix, non-official split, legacy metric (see `Model_Training_Reference.md` §2.22). Re-run built and tested (`Asif's/M30_v2/`, 53/53), **awaiting a Colab run**; it prints an admission-test verdict (PASSES / MARGINAL / FAILS) that decides whether item 3 stays selected | — |
