...
| 2 | Pre-trained DL model, one per member | 🟡 Two exist (M3 MobileNetV2 · Asif, M4 AST · Asif/Barshon). M1/M2 are **trained from scratch** — they do not count | Need ≥4 pre-trained backbones, one per member, all on the official split |
| 3 | ≥ X = 4 transformer models | 🔴 M4 (AST) is the only real transformer backbone and **has no committed `results_M4.json`**. M23 (AST+SpecAug, Farhana) undocumented. M33_v2 is a temporal transformer *head*, not a pre-trained transformer backbone. M37_v2 (LoRA-AST) unestablished | 4 distinct pre-trained transformers, fully logged |
