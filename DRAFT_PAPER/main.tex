...
detailed adventitious sound descriptions only poorly. A linear probe then separates the
two readings of that failure: a foundation model never exposed to respiratory audio
already encodes thirteen of our fourteen acoustic concepts, so the information is
present and the failure is one of reference rather than representation---and adapting
that model to the benchmark with a third of a percent of its parameters degrades
thirteen of the fourteen. We further report a non-replication of our own: an
interpretability cost we had recorded from a single random initialisation reverses sign
across five, and we withdraw it. We release the corrected split loader, a canonical
score format with paired significance tests, and the audit tool that detects all three
faults. The contribution is a measurement rather than a mechanism: concept-level
validation against these labels is bounded by the reliability of the labels themselves.
\end{abstract}
...
Section~\ref{subsec:conceptgate} is therefore not a representational failure, which is
the strongest available evidence that it is a referential one.
...
Seven limitations bound the claims above. The corrected-partition evidence currently
...
implementation, and a better one may exist; the probe of
Section~\ref{subsec:probe} narrows this---the information is demonstrably present in a
learned representation---but it does not establish that a hand-built detector can reach
0.65 on these labels, and we do not claim it. The reliability argument leans on two
published studies plus a single-clinician study still in flight, against the $n=7$ and
$n=12$ of the literature.
...
that seven senior physicians reproduce at a 47.77\% ICBHI score. A linear probe then
showed that the acoustic information those detectors were built to capture is present
and linearly accessible in a foundation model that has never heard a stethoscope, which
locates the failure at the labels rather than at the representation---and that adapting
that model to the benchmark degrades thirteen of the fourteen concepts, worst of all the
one concept that carries diagnostic signal. We also withdrew one of our own results: an
interpretability cost recorded from a single random initialisation reversed sign across
five, and a mechanism verdict was being produced by a sign test whose design floor made
it incapable of ever supporting the hypothesis. The contribution is a
